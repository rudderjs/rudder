...
Graduate to 1.0.0. The `image()` factory and `ImageProcessor` fluent API (resize, crop, fit, format, quality, lossless, stripMetadata, optimize, rotate, blur, grayscale, conversions) plus the terminal methods (toBuffer, toFile, toStream, toStorage, generateToStorage, metadata) are now part of the stable public API.
