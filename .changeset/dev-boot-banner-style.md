...
- `@rudderjs/vite` now splices `· Rudder vX.Y.Z` (in Rudder's brand orange) into
  Vike's startup line (`Vike v… · Vite v… · Rudder v1.5.1 · ready in N ms`),
  reading the installed `@rudderjs/core` version. Falls back to printing its own
  line if Vike's banner format changes, so the version is never lost. Dev-only.
