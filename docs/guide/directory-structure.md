...
A fresh `pnpm create rudder-app` scaffold ships only the directories above. Other conventional folders — `Jobs/`, `Services/`, `Events/`, `Listeners/`, `Mail/`, `Commands/`, `Notifications/`, `Agents/`, `Mcp/`, `Terminal/` — get created the first time you run the matching `make:*` command. See [Rudder Console](/guide/rudder) for the full list.
