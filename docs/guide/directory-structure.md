...
The framework generates a small, fixed set of files. The rule: **generated types live next to the thing they type and are committed** (so `tsc`/CI stay green with no generate step); **regenerable caches live in `bootstrap/cache/` and are gitignored**. Everything generated carries an `AUTO-GENERATED` header and a `__` path marker — never hand-edit them.

| File | What it is | Committed? | Regenerated by |
|---|---|---|---|
| `pages/__view/**` | Vike page stubs for `app/Views/**` (+ the typed-views `registry.d.ts`) | **yes** — Vike discovers pages via `git ls-files`; gitignoring 404s every view | views scanner, on every dev/build |
| `routes/__registry.d.ts` | `RouteRegistry` augmentation — typed `route(name, params)` from `.name()` chains | **yes** | routes scanner (dev/build) or `pnpm rudder routes:sync` |
| `app/Models/__schema/registry.d.ts` | Typed-models registry — column types introspected from the migrated schema (native engine) | **yes** — CI can't regenerate it (needs a live, migrated database) | `pnpm rudder migrate` / `pnpm rudder schema:types` |
| `bootstrap/cache/providers.json` | Provider auto-discovery manifest | **no** (gitignored) | `pnpm rudder providers:discover` |

Why not one generated directory (à la `.nuxt/`)? Those frameworks regenerate everything on postinstall, so they can gitignore the lot. Rudder's artifacts can't all be regenerated without context (the models registry needs your migrated database; the Vike stubs must be committed), and a registry sitting next to its domain — models types in `app/Models/`, route types in `routes/` — is where you'd look for it.
