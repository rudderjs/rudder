...
```ts
// .rudder/types/views.d.ts — AUTO-GENERATED; commit it
...
Run `pnpm dev` (or `pnpm build`) once — the scanner regenerates `.rudder/types/views.d.ts`, and the corresponding `view('settings', ...)` calls are now type-checked.
