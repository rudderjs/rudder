...
### Local test of unmerged `fix/hmr-model-reregister` (orm) — necessary but NOT sufficient (pilotiq agent, 2026-05-25)

Tested the unmerged branch `fix/hmr-model-reregister` (commit `cf28c8a0`, `@rudderjs/orm` patch: `ModelRegistry.register()` re-points at a re-imported model class + re-installs accessors, instead of no-op-on-collision) WITHOUT waiting for publish — built it in the rudder clone and linked it into the playground via a temporary `pnpm.overrides` `"@rudderjs/orm": "link:../rudder/packages/orm"`, `RUDDER_ORM_TRACE=1`.

- **The fix works for its stated purpose ✓.** The **dual class identity is GONE** — every `model=Article` line is now `class=#1` (zero `#2` across the whole run; was `#1`+`#2` on npm `orm@1.12.3`). So `register()` re-pointing does collapse the re-imported identity.
- **But the wedge STILL reproduces ✗.** flood `54 54 0 0 0 0 0 0 54 0` (3/10 full), `settled rows=0`, three single edits each → `rows=0`. The empty requests still issue only the badge `count rows=0` and **never the table `paginate`**; the 3 successful `paginate rows=6` are all on `adapter=#1` (the pre-reboot warm adapter). Post-reboot renders skip the table-data `paginate`.
- **Conclusion:** the dual-class identity was a **secondary symptom** — fixing it is necessary-but-not-sufficient. The actual REOPEN #2 wedge is the **post-reboot half-booted render not issuing the table-data `paginate`** (only the chrome/badge `count` runs) — i.e. pilotiq's **Vike `+data` SSR render** skipping the resource table-data branch, which neither #663 (quiesce barrier, covers `handleRequest()`) nor this orm re-register fix addresses. Worth merging (removes a real artifact), but it won't close REOPEN #2 alone.
- **Suggested fix direction:** ensure the table-data builder actually runs (paginate issued) for renders that land in the reboot window — i.e. extend the in-flight/quiesce coverage to the **Vike SSR `+data` render path**, not just `RudderJS.handleRequest()`. The "issues `count` (badge, via one path) but never `paginate` (table, via the +data path)" split is the tell that one render path is gated/drained and the other isn't.

### Framework-session trace — ruled OUT 3 causes; it's a runtime schema-build, not the re-boot lifecycle (rudder side, 2026-05-25 later)

⚠️ Correction to the "extend quiesce to +data" direction: the wedge is persistent on **single edits with no concurrent request** → there's no window to drain, so quiesce-coverage can't be it. Traced the full pilotiq render chain against source and ruled out the obvious suspects:

- **Stale panel — NO.** `PilotiqServiceProvider.register()` does `PilotiqRegistry.reset()` then `register(panel)`, and BOTH render paths re-resolve the live panel by name (`dispatchPageData → PilotiqRegistry.findByPath`; route handlers via `livePanel()`). Post-reboot the panel is **fresh**.
- **Model identity — NO** (#665 collapsed `class=#1/#2`; wedge persists).
- **Memoized schema — NO.** `callPageSchema` runs `PageClass.schema(ctx)` **fresh** per request.

Full chain: `dispatchPageData → findByPath → resourceIndexData → R = panel.findResource(slug) → R.pages.index (ListArticles) → callPageSchema → ListPage.schema(ctx)`, where `ListPage.schema` (pilotiq `defaultPages.ts:120`) is `const R = getResource(); const table = R.table(Table.make()); … return [ …, table ]`. **Every input (`ArticleResource`, `ArticlesTable`) is a stable imported class unchanged by an `AdminPanel.ts`-only edit** — so by static reading the Table element *should* be emitted. It isn't, post-reboot. That contradiction means the cause is a **runtime behavior** only visible in the reproducing env: `R.table()` building an empty/dropped table, `getResource()` resolving a half-built resource, or `resolveSchema` filtering the table out. Static analysis is exhausted here.

**Decisive 3-point probe (pilotiq-side, in the repro — compare a warm render vs a post-reboot wedged one):**
1. In `ListPage.schema` (`defaultPages.ts:120`): log `getResource()` identity, `R.table(Table.make())`'s **column count**, and whether the returned `elements` array contains a `table`.
2. In `resolveSchema`: log whether the table element **survives** resolution (a visibility/permission predicate dropping it?).
