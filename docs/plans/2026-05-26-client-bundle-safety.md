...
- ~~**Audit other client-reachable packages** against the gate~~ **DONE 2026-05-26.** Audited every entry views/client code imports: `@rudderjs/view`, `@rudderjs/router`, `@rudderjs/support`, `@rudderjs/ai`, `@rudderjs/middleware/client`, `@rudderjs/contracts` — all client-safe. Added the runtime-bearing ones (view, router, support, ai, middleware/client) to the gate `TARGETS` so they're permanently guarded, not just verified once. `@rudderjs/contracts` left out (types-only — `export *` yields no runtime). The `@rudderjs/core` view references found earlier were false positives (demo-data strings + a comment, not imports). **Net: the entire legitimately-client-reachable surface is client-safe and gated.**
