...
- ~~Named `whereIn`/`whereNotIn`, `whereBetween`/`whereNotBetween`, `whereNull`/`whereNotNull`, `whereColumn`, `whereNot`, date helpers (`whereDate`/`whereMonth`/…)~~ — shipped (whereX sugar + `whereColumn` PRs; `whereNot` group-negation + `whereDate`/`whereTime`/`whereDay`/`whereMonth`/`whereYear` native-engine, via the `Dialect.dateExtract` seam, Drizzle follow-up pending). Still missing: `wherePast`/`whereFuture`/`whereToday` relatives, `whereLike` w/ caseSensitive.
