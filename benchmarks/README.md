...
2. **Identical data.** A deterministic seeded PRNG (`src/prng.mjs`, no `Math.random()`) writes the rows ORM-neutrally — raw `better-sqlite3` on SQLite, raw porsager bulk inserts on Postgres, raw `mysql2` bulk inserts on MySQL — so no contender gets a seeding-path advantage. The same PRNG stream + fan-out drives all three engines, so the datasets are equivalent. Read benches open the untouched seed DB; write benches get a fresh scratch first (file copy on SQLite, `CREATE DATABASE … TEMPLATE` clone on Postgres, `CREATE TABLE … LIKE` + `INSERT … SELECT` on MySQL).
