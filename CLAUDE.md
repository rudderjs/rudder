...
- **Model instance methods don't work at runtime**: `@rudderjs/orm`'s Prisma adapter returns plain records, NOT Model instances. `await Model.where('id', x).first()` gives you a data object without prototype methods — `client.hasGrantType()` throws "is not a function". Use a `<pkg>/models/helpers.ts` pattern with standalone functions: `clientHelpers.hasGrantType(record, 'foo')`. Revoke via static `Model.update(id, { revoked: true })`, not `record.revoke()`. See `@rudderjs/passport` for the reference pattern.
