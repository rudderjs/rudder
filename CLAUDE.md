...
- **Route/bootstrap changes not picked up in dev**: `@rudderjs/vite`'s `rudderjs:routes` plugin watches `routes/` and `bootstrap/` and invalidates SSR modules on change. If this stops working, check that `server.environments.ssr.moduleGraph.invalidateAll()` is being called. Never use `server.restart()` — it closes the module runner and breaks in-flight SSR requests. GlobalThis singletons (`__rudderjs_instance__`, `__rudderjs_app__`) must also be cleared for re-bootstrap to create a fresh instance.
