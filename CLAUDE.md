...
Framework providers are auto-discovered from each package's `package.json` `rudderjs` field. The `defaultProviders()` helper reads `bootstrap/cache/providers.json` (generated by `pnpm rudder providers:discover`) and returns the right classes in stage + topo order — `foundation → infrastructure → feature → monitoring`, with `depends` resolved within each stage.

Run `pnpm rudder providers:discover` after installing or removing any framework package. The scaffolder runs it automatically on `--install`. The manifest is gitignored.
...
- **Route/bootstrap/app changes not picked up in dev**: `@rudderjs/vite`'s `rudderjs:routes` plugin watches `routes/`, `bootstrap/`, and `app/` and invalidates SSR modules on change. All three directories need full re-bootstrap (clearing `__rudderjs_instance__` + `__rudderjs_app__`) because `app/` files (models, resources, controllers) are captured in closures during provider boot. Never use `server.restart()` — it closes the module runner and breaks in-flight SSR requests.
- **Client IP showing `unknown`**: Use `req.ip` (set by server-hono's `extractIp()`), not raw headers. In dev mode, `@rudderjs/vite`'s `rudderjs:ip` plugin injects `x-real-ip` from `req.socket.remoteAddress` before universal-middleware converts to Web Request. Custom `.by()` rate-limit functions must read `req.ip`, not `req.headers['x-real-ip']`.
...
- **`whereHas` adapter requirements** (since Laravel parity #2 PR3): on Prisma, direct relations (`hasMany`/`hasOne`/`belongsTo`) need an `@relation` declared in `schema.prisma` with the same name — the adapter uses native `some`/`none`. Polymorphic and pivot relations route through a 2-step lookup so they work without a Prisma-declared relation. On Drizzle, every related table referenced from a `whereHas` call must be registered via `tables: { ... }` on `drizzle()` config or `DrizzleTableRegistry.register(name, table)`; missing tables surface a clear error. **`morphTo` cannot be used with `whereHas`** — the related table is dynamic; filter on `{morphName}Id` / `{morphName}Type` directly. Nested `whereHas` inside a constrain callback throws (deferred to v2). `withWhereHas` falls back to plain `with(relation)` on adapters that don't implement `withConstrained` (Drizzle today).
...
- **Static prerender opt-in is `export const prerender = true`** at the top of a view file (`app/Views/Foo.tsx`). The scanner reads it the same way it reads `export const route`, emits a `+prerender.ts` for Vike, and `pnpm build` writes static HTML to `dist/client/<url>/index.html`. Build-time only — dev still SSRs every request. The controller is NOT called at prerender time, so a prerendered view must render with no per-request props (build the demo around `view('id')` with no second arg; defaults inside the component handle anything else).
- **Typed `route(name, params)` lookups auto-populate**: the `@rudderjs/vite` routes scanner walks `routes/*.ts` for inline `.name('foo')` chains and emits `pages/__view/routes.d.ts` augmenting `RouteRegistry` — typo names + missing params fail `tsc`. Only literal-path AND literal-name chains are picked up; variable paths (`router.get(loginPath, ...).name(...)`) and runtime-registered routes (e.g. via `registerAuthRoutes(router, opts)`) are NOT auto-discovered — hand-augment the interface for those. Re-run on demand via `pnpm rudder routes:sync` (skip-boot, so it works before `pnpm dev` ever ran).
