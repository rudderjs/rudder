...
- **Recurring, one package per run.** Coverage rotates through every package; a full cycle takes
  roughly as many days as there are packages (~7 weeks at one/day).
- **Read-only analysis.** Auditor subagents read, grep, and reason. They do not run destructive
  commands, mutate the repo, or call external services beyond GitHub issue search/creation.
- **Anti-noise is the priority.** A tracker that fills with near-duplicate or low-value issues is
  worse than no tracker. The caps and dedup rules in §6 are hard requirements, not suggestions.
...
1. **Locate the tracker** — `gh issue list --label audit:tracker --state open`. If none exists,
   STOP and report (setup incomplete).
2. **Pick the package** — parse the tracker table; choose the row with the oldest `Last audited`
   date. `never` sorts before any date. Ties broken by the §9 priority order (security-sensitive
   first). Exactly one package per run.
3. **Fan out 6 auditors in parallel** — one read-only subagent per dimension (§4), each scoped to
   the chosen package directory and returning structured findings (§5).
4. **Dedup + cap** — for each finding, run the §6 gate. Drop duplicates and anything past the cap.
5. **File issues** — create the survivors with the §7 template and §8 labels.
6. **Update the tracker** — set the row's `Last audited` = today (pass the date in; the agent's
   clock may be unavailable), `Last findings` = count filed, `Dimensions clean` = dimensions that
   produced zero filed issues.
7. **Post a run summary** — a comment on the tracker: package, per-dimension counts, links to new
   issues, and anything skipped as a duplicate.
...
| 5 | **Code quality** (`audit:quality`) | Logic duplication (same algorithm in 2+ places), dead code, oversized/again-and-again functions, leaky abstractions. Must point at specific duplicated spans. | Pure style/formatting (lint owns that); subjective naming preferences. |
...
```
{
  ...
  rationale: "<why it matters + concrete impact>",
...
4. **Per-dimension cap: 2.** If a dimension yields more, keep the 2 highest-severity (ties → most
   concrete/actionable). Note the dropped count in the run summary.
5. **Per-run cap: 12** (6 dimensions × 2). Never exceed.
6. A dimension that produces zero survivors is recorded as **clean** in the tracker, NOT as an issue.
...
- A priority from the severity map (§9).

`audit:tracker` is reserved for the single rotation tracker issue — never on a finding.

---

## 9. Severity → priority & rotation seeding
...
After cycle 1, order is purely by oldest `Last audited` — seed order only matters for the first pass.
