 ...
 * `escapeHtml`, and `SafeString` so pulse doesn't need to pull
 * `@rudderjs/view` (which has a `vike` peer dep that's unwanted baggage
 * for a self-contained debug tool).
 *
 * Mirrors the same helper used by `@rudderjs/telescope` and
 * `@rudderjs/horizon`. If `@rudderjs/view` ever drops the Vike peer or
 * splits the HTML helpers into a separate package, this file can be
 * replaced with a one-line `export ... from '@rudderjs/view'`.
 *
 * Semantics:
