...
- feaee76: **Breaking:** IV encoding changed from hex to base64 to match Laravel's `Encrypter` wire format — existing 1.x ciphertexts must be re-encrypted after upgrading.
...
- 5a24833: Key-length guard on `CryptRegistry.set()`, zero-on-rotation fix, malformed-payload validation, `parseKey` base64 length check, and extracted `parsePayload`/`resolvedKeys` helpers.
