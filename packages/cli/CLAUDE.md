...
- `src/commands/make/` — 16 CLI-owned generators: controller, model, job, middleware, request, provider, command, event, exception, listener, mail, test, policy (`extends Policy` from `@rudderjs/auth`), observer (`implements ModelObserver` from `@rudderjs/orm`), cast (`implements CastUsing` from `@rudderjs/orm`, no suffix), notification (`extends Notification` from `@rudderjs/notification`). No `make:rule` / `make:scope` — Rudder validation is zod-via-FormRequest (no first-class `Rule`) and global scopes are inline `ScopeFn` functions in `static globalScopes` (no standalone `Scope` class); neither has an abstraction to scaffold against.
...
| CLI + all framework packages | `doctor` — green/yellow/red pre-flight across 42 checks. CLI ships `env` / `structure` / `deps` / `runtime` checks (incl. `structure:reflect-metadata`, `structure:tsconfig-decorators`, `deps:single-orm-driver`, `deps:single-vike-renderer`); framework packages contribute via a `./doctor` subpath export (see `load-package-checks.ts`) |
