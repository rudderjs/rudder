...
- 103d377: Harden the database queue worker and payload serializer against malformed and hostile payloads.

  - **The native `database` worker no longer crashes on a poison-pill row.** `_process` parsed and decoded the stored payload (`JSON.parse` + `decodePayload`) _before_ its try/catch, and the `work()` poll loop had no catch. A row whose payload was not valid JSON (corruption, a row written straight into a shared `jobs` table, or SQL injection elsewhere) threw synchronously, killed the worker, and — because the row stayed reserved — re-crashed every worker that reclaimed it after `retry_after`, stalling the whole queue. The parse/decode now runs fail-closed: an unparseable row is dead-lettered to `failed_jobs` and removed, and the loop catches unexpected reservation/processing errors (e.g. `SQLITE_BUSY` when multiple worker processes contend on one SQLite file) and keeps running.
...
- ad9721d: Two queue-worker correctness fixes:
