...
- `src/device-code-secret.ts` — `hashDeviceSecret()` (plain SHA-256 for device + user codes)
- `src/opaque-token.ts` — `newOpaqueToken()` / `hashOpaqueToken()` (refresh tokens + auth codes; `randomBytes(48).toString('base64url')` + plain SHA-256)
...
- **Keys from env or filesystem** — `PASSPORT_PRIVATE_KEY`/`PASSPORT_PUBLIC_KEY` env vars, or files at `storage/oauth-{private,public}.key`. After a `passport:keys --force` rotation, the prior public key is also retained at `storage/oauth-previous-public.key` for the JWKS-style verification grace window — see the rotation pitfall below.
...
- **Device codes rate-limited** — initial 5-second polling interval enforced server-side; escalates by 5s per `slow_down` response per RFC 8628 §3.5. The current interval is persisted on the row and forwarded in the `slow_down` response body so well-behaved clients use the new value directly instead of guessing. The escalation cap defaults to 60s and is configurable via `Passport.deviceMaxInterval(seconds)` or `PassportConfig.deviceMaxInterval` — values below 5 are clamped to the 5s floor (escalation always needs to be able to take effect). Raise it for machine-only / no-human-in-the-loop device flows where misbehaving clients warrant aggressive back-off.
...
- **Refresh tokens and auth codes hashed at rest** — `oauth_refresh_tokens` and `oauth_auth_codes` each carry a `tokenHash` column (SHA-256 of the plaintext bearer secret). The plaintext returned to the client is freshly generated CSPRNG `randomBytes(48).toString('base64url')` (384 bits, 64 chars URL-safe), **not** the row's cuid `id`. The row's `id` is purely internal — referenced only by `accessTokenId` linkage (refresh tokens) and family-revocation walks. `refreshTokenGrant` and `exchangeAuthCode` hash their inbound plaintext and look up by `where('tokenHash', hash)`. Same shape as device codes — a DB read leak yields hashes, not usable bearer credentials. `hashOpaqueToken` + `newOpaqueToken` are exported from the package main entry. Pre-migration credentials (where the plaintext was the row id) stop working at deploy time; see "Pitfalls".
- **Personal access tokens** — auto-create an internal `__personal_access__` OAuth client on first use
- **Routes split between web and api groups** — the consent flow (`GET/POST/DELETE /oauth/authorize` + `DELETE /oauth/tokens/:id`) belongs on the **web** group because it depends on session + authenticated user resolution. The token + `POST /oauth/revoke` (RFC 7009) + `POST /oauth/token/introspect` (RFC 7662) + device + scopes endpoints are stateless and belong on **api**. Use `registerPassportWebRoutes()` in `routes/web.ts` and `registerPassportApiRoutes()` in `routes/api.ts`. The original `registerPassportRoutes()` mounts everything on a single router and is kept for back-compat / single-group apps.
- **Two ways to revoke** — `DELETE /oauth/tokens/:id` is the by-database-id, **user-bearer-authenticated** route (a logged-in user revoking their own token from a settings page). `POST /oauth/revoke` is the **RFC 7009**, by-token-value, **client-authenticated** route that third-party OAuth SDKs call on logout (revoke by value, confidential-client auth, always HTTP 200 per §2.2, ownership-scoped so a client can't revoke another's token). Both funnel through `routes/revoke-chain.ts::revokeAccessTokenChain()` so the access-token-plus-refresh-family cascade lives in one place. Confidential-client auth for the machine-to-machine endpoints goes through `routes/helpers.ts::authenticateConfidentialClient()` (wraps `resolveClientCredentials` + `verifyConfidentialCredentials`).
- **Introspection is NOT ownership-scoped** — `POST /oauth/token/introspect` (RFC 7662, `routes/introspect.ts`) lets any authenticated confidential client introspect ANY token, because a resource server legitimately validates access tokens issued to OTHER clients (its API consumers). That's the deliberate contrast with `POST /oauth/revoke`, which IS ownership-scoped. Introspection reflects live state: `active: false` for a bad-signature / expired / revoked / unknown token; `scope` comes from the live DB row (not the JWT claim), same authority as bearer middleware. A malformed token is `{ active: false }` + 200, never an error.
...
- **Rotating the RSA keypair carries a JWKS-style grace window.** `rudder passport:keys --force` writes a new private/public pair (timestamped audit backups in `*.bak.<ISO-timestamp>`, plus a rolling `oauth-previous-public.key` for the verifier). Every new JWT carries a `kid` header equal to the SHA-256 fingerprint of the public key that signed it; `verifyToken()` walks `[currentPublicKey, ...previousPublicKeys]` and accepts a match against any retained key. Tokens minted before the rotation keep verifying until they expire naturally (default 15 days for access tokens) — no global sign-out at rotation time. Drop `oauth-previous-public.key` (or call `Passport.setPreviousPublicKey(null)`) once the old tokens have expired to close the grace window. Single previous-slot is intentional: one rotation deep — operators who need a longer history should stage rotations to land outside the configured access-token lifetime, at which point old tokens have expired anyway. Legacy tokens (minted before the JWKS PR) carry no `kid` and fall through to "try each verification key in order" — same compat pattern as `iss` (P7).
...
- **Migrating to hashed refresh tokens / auth codes invalidates every in-flight credential.** Pre-migration, the plaintext returned to clients was the row's cuid `id`; post-migration, it's freshly generated CSPRNG hex with the SHA-256 stored in `tokenHash`. Old refresh tokens cannot match the new lookup (their hashed form was never persisted), so affected sessions force-relogin on next refresh. Auth codes drain in 10 minutes naturally. Same blast radius as rotating the RSA keypair — plan as a coordinated sign-out window. One-time migration; not a recurring concern. Once shipped, the contract is durable — refresh-token / auth-code rotation is a normal operation, not a credential-invalidating event.
