...
import { Channel, PrivateChannel, PresenceChannel, initWsServer, resetWs, getUpgradeHandler, broadcast, wsStats, registerAuth, } from './index.js';
// ─── Helpers ───────────────────────────────────────────────
...
describe('WebSocket connection', () => {
    it('connected message with socketId', () => withServer(async (port) => {
        const ws = await openWs(port);
        const msg = await nextMsg(ws);
        assert.strictEqual(msg['type'], 'connected');
        assert.ok(typeof msg['socketId'] === 'string' && msg['socketId'].length > 0);
        ws.terminate();
    }));
    ...
    it('invalid JSON → error', () => withServer(async (port) => {
        const ws = await openAndConsume(port);
        const p = nextMsg(ws);
        ws.send('not-json');
    ...
    it('wsStats reflects open connections', () => withServer(async (port) => {
        const ws = await openAndConsume(port);
...
describe('public channel', () => {
    it('subscribe returns subscribed confirmation', () => withServer(async (port) => {
        const ws = await openAndConsume(port);
    ...
    it('broadcast() delivers to subscriber', () => withServer(async (port) => {
        const ws = await openAndConsume(port);
    ...
    it('broadcast() does not reach non-subscribers', () => withServer(async (port) => {
    ...
    it('unsubscribe stops delivery', () => withServer(async (port) => {
        const ws = await openAndConsume(port);
    ...
    it('wsStats reflects active channels after subscribe', () => withServer(async (port) => {
        const ws = await openAndConsume(port);
        await send(ws, { type: 'subscribe', channel: 'stats-test' });
        assert.ok(wsStats().channels >= 1);
        ws.terminate();
    }));
    it('client-event forwarded to other subscribers, not sender', () => withServer(async (port) => {
    ...
    it('client-event to unsubscribed channel returns error', () => withServer(async (port) => {
        const ws = await openAndConsume(port);
...
describe('private channel', () => {
    it('denied when no auth handler registered', () => withServer(async (port) => {
        const ws = await openAndConsume(port);
    ...
    it('denied when auth returns false', () => withServer(async (port) => {
    ...
    it('allowed when auth returns true', () => withServer(async (port) => {
    ...
    it('auth callback receives token from subscribe message', () => withServer(async (port) => {
        let got;
    ...
    it('wildcard pattern matches multiple channels', () => withServer(async (port) => {
    ...
    it('broadcast reaches authenticated subscriber', () => withServer(async (port) => {
...
describe('presence channel', () => {
    it('joiner receives presence.members after subscribed', () => withServer(async (port) => {
    ...
    it('existing member gets presence.joined when second joins', () => withServer(async (port) => {
    ...
    it('remaining member gets presence.left on disconnect', () => withServer(async (port) => {
