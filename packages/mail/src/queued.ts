...
export async function dispatchMailJob(
  ...
  try {
    ...
    throw new Error(
      '[Rudder Mail] Queued mail requires @rudderjs/queue. Install it with: pnpm add @rudderjs/queue'
    )
  }

  const adapter = QueueRegistry.get()
  if (!adapter) {
    throw new Error('[Rudder Mail] No queue adapter registered. Add queue() to providers.')
  }
  ...
  const job = {
    handle: async () => {
      ...
      if (!mailAdapter) {
        throw new Error('[Rudder Mail] No mail adapter registered. Add mail() to providers.')
      }
