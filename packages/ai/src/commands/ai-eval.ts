...
import {
  ...
  readFixture,
  writeFixture,
} from '@gemstack/ai-sdk/eval'
import type { EvalSuite, EvalCase, Metric, SuiteReport, SuiteJson } from '@gemstack/ai-sdk/eval'
import { AiFake, type Agent, type AgentResponse } from '@gemstack/ai-sdk'

// `AiFakeStep` is not on the public `@gemstack/ai-sdk` surface; derive it from
// the public `stepsFromResponse` return type so record/replay stays typed
// without reaching into engine internals.
type AiFakeStep = ReturnType<typeof stepsFromResponse>[number]
...
export function parseArgs(args: string[]): AiEvalOptions {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (VALUE_FLAGS.has(name)) {
      ...
      if (!value) throw new Error(`[ai:eval] ${name} requires a value`)
...
function parsePattern(pattern: string): { root: string; suffix: string } {
  ...
  if (!postfix.startsWith('*')) {
    throw new Error(
      `[ai:eval] Unsupported eval pattern "${pattern}". ` +
