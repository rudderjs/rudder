...
- `src/native/` — the engine: `compiler.ts` (SQL compiler, shared positional `Bindings`), `dialect.ts` + `dialect-pg.ts` + `dialect-mysql.ts`, `driver.ts` (seam; `AffectingExecutor` = MySQL no-RETURNING path), `drivers/` (lazy-load their optional-peer package), `query-builder.ts` (`NativeQueryBuilder`), `adapter.ts` (`NativeAdapter`, dev-HMR client cache), `replica-picker.ts` (`ReadReplicaPicker`/`makeReplicaPicker` — read/write-split replica selection, shared with orm-drizzle via the `@rudderjs/orm/native` shim), `isolation.ts` (`transaction(fn, { isolationLevel })` support — the lowercase-ANSI → SQL-keyword map IS the injection gate, the level is spliced never bound; pg emits `SET TRANSACTION ISOLATION LEVEL` as the FIRST statement inside BEGIN, mysql must emit it BEFORE `beginTransaction` — the un-scoped form applies to the NEXT transaction only, so the pooled connection releases clean; sqlite + savepoint scopes throw), `schema/` (Blueprint, DDL compiler, `SchemaBuilder`, `Migration`/`Schema`/`Migrator`, introspection, schema→TS type generator). ALTER + fresh are dialect-real since the P2-11 live arc: `SchemaBuilder.allTables()`/`dropAllTables()` are the dialect-aware catalog sweep (`migrate:fresh` no longer reads `sqlite_master` on pg/mysql; pg drops `CASCADE`, mysql toggles `FOREIGN_KEY_CHECKS`), alter-time FKs emit `ADD CONSTRAINT`/`DROP CONSTRAINT|DROP FOREIGN KEY` on pg/mysql (they were silently dropped before), and `dropIndex` is table-scoped on mysql (`DROP INDEX … ON t`). Live suites `schema/migrator-live.test.ts` (runs in an ISOLATED namespace — dedicated pg schema via porsager `connection.search_path` / dedicated mysql database — because `dropAllTables` would sweep parallel test files' tables on the shared CI db) + `schema/alter-live.test.ts`.
