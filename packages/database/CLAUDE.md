...
- `src/native/` — the engine: `compiler.ts` (SQL compiler, shared positional `Bindings`), `dialect.ts` + `dialect-pg.ts` + `dialect-mysql.ts`, `driver.ts` (seam; `AffectingExecutor` = MySQL no-RETURNING path), `drivers/` (lazy-load their optional-peer package), `query-builder.ts` (`NativeQueryBuilder`), `adapter.ts` (`NativeAdapter`, dev-HMR client cache), `schema/` (Blueprint, DDL compiler, `SchemaBuilder`, `Migration`/`Schema`/`Migrator`, introspection, schema→TS type generator).
