...
function walk(dir: string, extensions: string[], base = dir): string[] {
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    // Skip symlinks — a symlinked file (or dir) under app/Views/ could point
    // OUT of the app tree, and we'd ingest an arbitrary out-of-tree file as a
    // view and generate a page that imports it through the App/ alias. Real
    // views are plain files; ignoring links closes the escape with no cost.
...
/**
 * The id, import path, and route URL of a discovered view are interpolated
 * verbatim into single-quoted string contexts inside generated page modules
 * (`+Page.tsx`, `+route.ts`, the typed registry). A quote, backtick,
 * backslash, newline, or control char in any of them would break out of that
 * string and corrupt — or inject code into — the generated module. Legitimate
 * views (PascalCase filenames, slash-delimited routes) never contain these;
...
function discover(viewsRoot: string, pagesRoot: string, framework: Framework): DiscoveredView[] {
  ...
  return walk(viewsRoot, exts).map(rel => {
    ...
    // The id / importPath / route-url are spliced verbatim into generated page
    // modules. Skip (loudly) any view whose name or route override carries a
    // quote, backslash, newline, or control char — emitting it would corrupt
    // or inject code into the generated source. Legitimate views never trip
    // this; see CODEGEN_UNSAFE_RE.
    if (isCodegenSafe(view.id) && isCodegenSafe(view.importPath) && isCodegenSafe(view.url)) {
      return true
    }
    console.warn(
 ...
 * `viewProps` from server pageContext to the client. Without this, the server
 * SSR works but hydration crashes because the client component receives
 * `props = {}` (viewProps is undefined client-side).
 *
 * `viewHeaders` is deliberately NOT passed to the client: it is read only by
 * the SERVER-side `+headersResponse` hook (server pageContext always carries
 * it), and serializing it into the hydration payload leaked every controller
 * response-header value — including per-request CSP nonces — into client JS
 * and page source for no consumer.
 */
...
/**
 * RSC variant of the view-root config. Wires the Rudder framework hooks as
...
// Forwards controller viewProps to client hydration, and wires the Rudder
