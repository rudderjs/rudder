...
interface DiscoveredView {
  ...
  hasProps:    boolean
  /**
   * True when the view source exports `export const prerender = true` —
   * opts this view into Vike's build-time static prerender pipeline. The
   * scanner emits a `+prerender.ts` next to the generated `+Page.*` so
   * `vike build` writes the pre-rendered HTML to `dist/client/<url>.html`.
   *
   * Build-time only — dev still SSRs every request. Suitable for views
   * with no per-request data (landing pages, docs index, terms / privacy).
   */
  prerender:   boolean
}
...
/**
 * Detect `export const prerender = true` in a view source — same regex shape
 * as `ROUTE_EXPORT_RE`, tolerates `: boolean` annotation. Only the literal
 * `true` opts in; `false` or any expression is treated as "not opted in"
 * because Vike's `+prerender` boolean is a build-time directive, not a
 * runtime value.
 */
const PRERENDER_EXPORT_RE = /(?:^|[\s;])export\s+const\s+prerender\s*(?::\s*boolean)?\s*=\s*true\b/m

function readPrerenderOpt(absPath: string): boolean {
  try {
    return PRERENDER_EXPORT_RE.test(fs.readFileSync(absPath, 'utf8'))
  } catch {
    return false
  }
}
...
function generate(generatedRoot: string, pagesRoot: string, views: DiscoveredView[], framework: Framework): void {
  ...
  for (const v of views) {
    ...
    const prerenderFile = path.join(v.outDir, '+prerender.ts')
    if (v.prerender) {
      writeIfChanged(prerenderFile, PRERENDER_FILE_SOURCE)
    } else if (fs.existsSync(prerenderFile)) {
      // Source removed `export const prerender = true` since the last scan —
      // drop the generated file so Vike's build no longer prerenders.
