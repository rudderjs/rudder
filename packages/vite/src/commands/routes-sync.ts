 ...
 *
 * Regenerates `.rudder/types/routes.d.ts` from `routes/*.ts` without booting
...
export function registerRoutesSyncCommand(
  ...
  rudder.command('routes:sync', async (args: string[]) => {
    ...
    try {
      ...
      console.log(`✓ Scanned ${result.routeCount} named route${result.routeCount === 1 ? '' : 's'} → .rudder/types/routes.d.ts`)
    } catch (err) {
