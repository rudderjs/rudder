...
describe('routes-scanner — syncRoutesFromDisk', () => {
  ...
  it('writes the registry to .rudder/types/routes.d.ts and returns the count', () => {
    root = scaffold()
    ...
    const out = fs.readFileSync(path.join(root, '.rudder', 'types', 'routes.d.ts'), 'utf8')
    assert.match(out, /'about':\s*'\/about'/)
  })

  it('removes both legacy emit locations on write (migration)', () => {
    root = scaffold()
    write(root, 'routes/web.ts', `Route.get('/about', h).name('about')`)
    write(root, 'routes/__registry.d.ts', `// stale 2026-06 emit`)
    write(root, 'pages/__view/routes.d.ts', `// stale original emit`)
    process.chdir(root)
    syncRoutesFromDisk()
    assert.equal(fs.existsSync(path.join(root, 'routes', '__registry.d.ts')), false)
    assert.equal(fs.existsSync(path.join(root, 'pages', '__view', 'routes.d.ts')), false)
    assert.equal(fs.existsSync(path.join(root, '.rudder', 'types', 'routes.d.ts')), true)
  })

  it('writes the .rudder/README.md alongside the registry', () => {
    root = scaffold()
    write(root, 'routes/web.ts', `Route.get('/about', h).name('about')`)
    process.chdir(root)
    syncRoutesFromDisk()
    const readme = fs.readFileSync(path.join(root, '.rudder', 'README.md'), 'utf8')
    assert.match(readme, /generated by RudderJS/)
    assert.match(readme, /types\/routes\.d\.ts/)
  })
  ...
  it('is idempotent across re-runs', () => {
    ...
    const second = syncRoutesFromDisk()           // registry now on disk
