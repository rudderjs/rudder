...
export function getTemplates(ctx: TemplateContext): Record<string, string> {
  ...
  if (ctx.withAuth) {
    files['pages/login/+config.ts']        = authPageConfig(ctx)
    files['pages/login/+guard.ts']         = authGuard('login')
    files[`pages/login/+Page${ext}`]       = authLoginPage(ctx)
    files['pages/register/+config.ts']     = authPageConfig(ctx)
    files['pages/register/+guard.ts']      = authGuard('register')
    files[`pages/register/+Page${ext}`]    = authRegisterPage(ctx)
  }
...
function authPageConfig(ctx: TemplateContext): string {
  switch (ctx.primary) {
    case 'vue':
      return `import type { Config } from 'vike/types'
import vikeVue from 'vike-vue/config'
...
function authGuard(page: 'login' | 'register'): string {
  const comment = page === 'login'
    ? '// Already logged in — redirect to home'
    : '// Already registered and logged in — redirect to home'
  return `import { redirect } from 'vike/abort'
...
function authLoginPage(ctx: TemplateContext): string {
  switch (ctx.primary) {
    case 'vue':   return authLoginPageVue(ctx)
    case 'solid': return authLoginPageSolid(ctx)
    default:      return authLoginPageReact(ctx)
  }
}

function authRegisterPage(ctx: TemplateContext): string {
  switch (ctx.primary) {
    case 'vue':   return authRegisterPageVue(ctx)
    case 'solid': return authRegisterPageSolid(ctx)
    default:      return authRegisterPageReact(ctx)
  }
}

// ── React ────────────────────────────────────────────────────

function authReactStyles(ctx: TemplateContext): string {
  if (ctx.tailwind) return ''
  return `
const s = {
  wrap:  { display: 'flex', minHeight: '100svh', alignItems: 'center', justifyContent: 'center', padding: '1rem' } as React.CSSProperties,
  inner: { width: '100%', maxWidth: '360px' } as React.CSSProperties,
  card:  { border: '1px solid #e5e7eb', borderRadius: '8px', padding: '1.5rem', boxShadow: '0 1px 3px rgba(0,0,0,.1)' } as React.CSSProperties,
  label: { display: 'block', fontSize: '14px', fontWeight: 500, marginBottom: '4px' } as React.CSSProperties,
  input: { width: '100%', border: '1px solid #d1d5db', borderRadius: '6px', padding: '8px 12px', fontSize: '14px', boxSizing: 'border-box' as const, marginBottom: '1rem' } as React.CSSProperties,
  btn:   { width: '100%', background: '#111', color: '#fff', border: 'none', borderRadius: '6px', padding: '10px', fontSize: '14px', fontWeight: 500, cursor: 'pointer' } as React.CSSProperties,
  error: { background: '#fef2f2', color: '#dc2626', borderRadius: '6px', padding: '8px 12px', fontSize: '14px', marginBottom: '1rem' } as React.CSSProperties,
  link:  { textAlign: 'center' as const, fontSize: '14px', color: '#6b7280', marginTop: '1rem' } as React.CSSProperties,
}
`
}

function authLoginPageReact(ctx: TemplateContext): string {
...
export default function LoginPage() {
  ...
  return (
    <div ${tw ? 'className="flex min-h-svh items-center justify-center p-4"' : 'style={s.wrap}'}>
      <div ${tw ? 'className="w-full max-w-sm space-y-6"' : 'style={s.inner}'}>
        <div ${tw ? 'className="text-center"' : ''}>
          <h1 ${tw ? 'className="text-2xl font-bold"' : ''}>Welcome back</h1>
          <p ${tw ? 'className="text-sm text-gray-500 mt-1"' : 'style={{ color: "#6b7280" }}'}>Sign in to your account</p>
        </div>
        <form onSubmit={handleSubmit} ${tw ? 'className="space-y-4 rounded-lg border p-6 shadow-sm"' : 'style={s.card}'}>
          {error && <p ${tw ? 'className="rounded-md bg-red-50 px-3 py-2 text-sm text-red-600"' : 'style={s.error}'}>{error}</p>}
          <div>
            <label ${tw ? 'className="block text-sm font-medium mb-1"' : 'style={s.label}'} htmlFor="email">Email</label>
            <input
              id="email" type="email" placeholder="you@example.com"
              value={email} onChange={e => setEmail(e.target.value)}
              required autoComplete="email"
              ${tw ? 'className="w-full rounded-md border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-black"' : 'style={s.input}'}
            />
          </div>
          <div>
            <label ${tw ? 'className="block text-sm font-medium mb-1"' : 'style={s.label}'} htmlFor="password">Password</label>
            <input
          ...
          <button type="submit" disabled={loading}
            ${tw ? 'className="w-full rounded-md bg-black px-4 py-2 text-sm font-medium text-white hover:bg-black/90 disabled:opacity-50"' : 'style={s.btn}'}>
            {loading ? 'Signing in…' : 'Sign in'}
          </button>
          <p ${tw ? 'className="text-center text-sm text-gray-500"' : 'style={s.link}'}>
            Don't have an account?{' '}
            <a href="/register" ${tw ? 'className="underline hover:text-black"' : ''}>Register</a>
          </p>
        </form>
      </div>
    </div>
...
function authRegisterPageReact(ctx: TemplateContext): string {
  const tw = ctx.tailwind
  return `import { useState } from 'react'
${authReactStyles(ctx)}
export default function RegisterPage() {
  ...
  return (
    <div ${tw ? 'className="flex min-h-svh items-center justify-center p-4"' : 'style={s.wrap}'}>
      <div ${tw ? 'className="w-full max-w-sm space-y-6"' : 'style={s.inner}'}>
        <div ${tw ? 'className="text-center"' : ''}>
          <h1 ${tw ? 'className="text-2xl font-bold"' : ''}>Create an account</h1>
          <p ${tw ? 'className="text-sm text-gray-500 mt-1"' : 'style={{ color: "#6b7280" }}'}>Get started in seconds</p>
        </div>
        <form onSubmit={handleSubmit} ${tw ? 'className="space-y-4 rounded-lg border p-6 shadow-sm"' : 'style={s.card}'}>
          {error && <p ${tw ? 'className="rounded-md bg-red-50 px-3 py-2 text-sm text-red-600"' : 'style={s.error}'}>{error}</p>}
          <div>
            <label ${tw ? 'className="block text-sm font-medium mb-1"' : 'style={s.label}'} htmlFor="name">Name</label>
            <input
          ...
          <p ${tw ? 'className="text-center text-sm text-gray-500"' : 'style={s.link}'}>
            ...
            <a href="/login" ${tw ? 'className="underline hover:text-black"' : ''}>Sign in</a>
          </p>
        </form>
      </div>
    </div>
...
function authLoginPageVue(ctx: TemplateContext): string {
  const tw = ctx.tailwind
  return `<script setup lang="ts">
import { ref } from 'vue'
...
<template>
  <div ${tw ? 'class="flex min-h-svh items-center justify-center p-4"' : 'style="display:flex;min-height:100svh;align-items:center;justify-content:center;padding:1rem"'}>
    <div ${tw ? 'class="w-full max-w-sm space-y-6"' : 'style="width:100%;max-width:360px"'}>
      <div ${tw ? 'class="text-center"' : ''}>
        <h1 ${tw ? 'class="text-2xl font-bold"' : ''}>Welcome back</h1>
        <p ${tw ? 'class="text-sm text-gray-500 mt-1"' : 'style="color:#6b7280"'}>Sign in to your account</p>
      </div>
      <form @submit.prevent="handleSubmit" ${tw ? 'class="space-y-4 rounded-lg border p-6 shadow-sm"' : 'style="border:1px solid #e5e7eb;border-radius:8px;padding:1.5rem"'}>
        <p v-if="error" ${tw ? 'class="rounded-md bg-red-50 px-3 py-2 text-sm text-red-600"' : 'style="background:#fef2f2;color:#dc2626;border-radius:6px;padding:8px 12px;font-size:14px"'}>{{ error }}</p>
        <div>
          <label ${tw ? 'class="block text-sm font-medium mb-1"' : ''} for="email">Email</label>
          <input id="email" v-model="email" type="email" placeholder="you@example.com"
            required autocomplete="email"
            ${tw ? 'class="w-full rounded-md border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-black"' : 'style="width:100%;border:1px solid #d1d5db;border-radius:6px;padding:8px 12px;font-size:14px;box-sizing:border-box"'} />
        </div>
        <div>
          <label ${tw ? 'class="block text-sm font-medium mb-1"' : ''} for="password">Password</label>
          <input id="password" v-model="password" type="password" placeholder="••••••••"
            required autocomplete="current-password"
            ${tw ? 'class="w-full rounded-md border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-black"' : 'style="width:100%;border:1px solid #d1d5db;border-radius:6px;padding:8px 12px;font-size:14px;box-sizing:border-box"'} />
        </div>
        <button type="submit" :disabled="loading"
          ${tw ? 'class="w-full rounded-md bg-black px-4 py-2 text-sm font-medium text-white hover:bg-black/90 disabled:opacity-50"' : 'style="width:100%;background:#111;color:#fff;border:none;border-radius:6px;padding:10px;font-size:14px;cursor:pointer"'}>
          {{ loading ? 'Signing in…' : 'Sign in' }}
        </button>
        <p ${tw ? 'class="text-center text-sm text-gray-500"' : 'style="text-align:center;font-size:14px;color:#6b7280;margin-top:1rem"'}>
          Don't have an account?
          <a href="/register" ${tw ? 'class="underline hover:text-black"' : ''}>Register</a>
        </p>
      </form>
    </div>
  </div>
...
function authRegisterPageVue(ctx: TemplateContext): string {
  const tw = ctx.tailwind
  return `<script setup lang="ts">
import { ref } from 'vue'
...
<template>
  <div ${tw ? 'class="flex min-h-svh items-center justify-center p-4"' : 'style="display:flex;min-height:100svh;align-items:center;justify-content:center;padding:1rem"'}>
    <div ${tw ? 'class="w-full max-w-sm space-y-6"' : 'style="width:100%;max-width:360px"'}>
      <div ${tw ? 'class="text-center"' : ''}>
        <h1 ${tw ? 'class="text-2xl font-bold"' : ''}>Create an account</h1>
        <p ${tw ? 'class="text-sm text-gray-500 mt-1"' : 'style="color:#6b7280"'}>Get started in seconds</p>
      </div>
      <form @submit.prevent="handleSubmit" ${tw ? 'class="space-y-4 rounded-lg border p-6 shadow-sm"' : 'style="border:1px solid #e5e7eb;border-radius:8px;padding:1.5rem"'}>
        <p v-if="error" ${tw ? 'class="rounded-md bg-red-50 px-3 py-2 text-sm text-red-600"' : 'style="background:#fef2f2;color:#dc2626;border-radius:6px;padding:8px 12px;font-size:14px"'}>{{ error }}</p>
        <div>
          <label ${tw ? 'class="block text-sm font-medium mb-1"' : ''} for="name">Name</label>
        ...
        <p ${tw ? 'class="text-center text-sm text-gray-500"' : 'style="text-align:center;font-size:14px;color:#6b7280;margin-top:1rem"'}>
          Already have an account?
          <a href="/login" ${tw ? 'class="underline hover:text-black"' : ''}>Sign in</a>
        </p>
      </form>
    </div>
  </div>
...
function authSolidStyles(): string {
  return `
const s = {
  wrap:  { display: 'flex', 'min-height': '100svh', 'align-items': 'center', 'justify-content': 'center', padding: '1rem' },
  inner: { width: '100%', 'max-width': '360px' },
  card:  { border: '1px solid #e5e7eb', 'border-radius': '8px', padding: '1.5rem', 'box-shadow': '0 1px 3px rgba(0,0,0,.1)' },
  label: { display: 'block', 'font-size': '14px', 'font-weight': 500, 'margin-bottom': '4px' },
  input: { width: '100%', border: '1px solid #d1d5db', 'border-radius': '6px', padding: '8px 12px', 'font-size': '14px', 'box-sizing': 'border-box', 'margin-bottom': '1rem' },
  btn:   { width: '100%', background: '#111', color: '#fff', border: 'none', 'border-radius': '6px', padding: '10px', 'font-size': '14px', 'font-weight': 500, cursor: 'pointer' },
  error: { background: '#fef2f2', color: '#dc2626', 'border-radius': '6px', padding: '8px 12px', 'font-size': '14px', 'margin-bottom': '1rem' },
  muted: { color: '#6b7280' },
  link:  { 'text-align': 'center', 'font-size': '14px', color: '#6b7280', 'margin-top': '1rem' },
}
`
}

function authLoginPageSolid(ctx: TemplateContext): string {
  const tw = ctx.tailwind
...
export default function LoginPage() {
  ...
  return (
    <div ${tw ? 'class="flex min-h-svh items-center justify-center p-4"' : 'style={s.wrap}'}>
      <div ${tw ? 'class="w-full max-w-sm space-y-6"' : 'style={s.inner}'}>
        <div ${tw ? 'class="text-center"' : ''}>
          <h1 ${tw ? 'class="text-2xl font-bold"' : ''}>Welcome back</h1>
          <p ${tw ? 'class="text-sm text-gray-500 mt-1"' : 'style={s.muted}'}>Sign in to your account</p>
        </div>
        <form onSubmit={handleSubmit} ${tw ? 'class="space-y-4 rounded-lg border p-6 shadow-sm"' : 'style={s.card}'}>
          {error() && <p ${tw ? 'class="rounded-md bg-red-50 px-3 py-2 text-sm text-red-600"' : 'style={s.error}'}>{error()}</p>}
          <div>
            <label ${tw ? 'class="block text-sm font-medium mb-1"' : 'style={s.label}'} for="email">Email</label>
            <input id="email" type="email" placeholder="you@example.com"
              value={email()} onInput={e => setEmail(e.currentTarget.value)}
              required autocomplete="email"
              ${tw ? 'class="w-full rounded-md border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-black"' : 'style={s.input}'} />
          </div>
          <div>
            <label ${tw ? 'class="block text-sm font-medium mb-1"' : 'style={s.label}'} for="password">Password</label>
          ...
          <button type="submit" disabled={loading()}
            ${tw ? 'class="w-full rounded-md bg-black px-4 py-2 text-sm font-medium text-white hover:bg-black/90 disabled:opacity-50"' : 'style={s.btn}'}>
            {loading() ? 'Signing in…' : 'Sign in'}
          </button>
          <p ${tw ? 'class="text-center text-sm text-gray-500"' : 'style={s.link}'}>
            Don't have an account?{' '}
            <a href="/register" ${tw ? 'class="underline hover:text-black"' : ''}>Register</a>
          </p>
        </form>
...
function authRegisterPageSolid(ctx: TemplateContext): string {
  const tw = ctx.tailwind
  return `import { createSignal } from 'solid-js'
${tw ? '' : authSolidStyles()}
export default function RegisterPage() {
  ...
  return (
    <div ${tw ? 'class="flex min-h-svh items-center justify-center p-4"' : 'style={s.wrap}'}>
      <div ${tw ? 'class="w-full max-w-sm space-y-6"' : 'style={s.inner}'}>
        <div ${tw ? 'class="text-center"' : ''}>
          <h1 ${tw ? 'class="text-2xl font-bold"' : ''}>Create an account</h1>
          <p ${tw ? 'class="text-sm text-gray-500 mt-1"' : 'style={s.muted}'}>Get started in seconds</p>
        </div>
        <form onSubmit={handleSubmit} ${tw ? 'class="space-y-4 rounded-lg border p-6 shadow-sm"' : 'style={s.card}'}>
          {error() && <p ${tw ? 'class="rounded-md bg-red-50 px-3 py-2 text-sm text-red-600"' : 'style={s.error}'}>{error()}</p>}
          <div>
            <label ${tw ? 'class="block text-sm font-medium mb-1"' : 'style={s.label}'} for="name">Name</label>
